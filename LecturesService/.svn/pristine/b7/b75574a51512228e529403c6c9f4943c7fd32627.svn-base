...
<body>
	<nav class="breadcrumb">
		...
		讲座反馈信息 <span class="c-gray en">&gt;</span> 报名管理 <a
	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="exportExcel()"
				...
				onclick="enroll_add('添加报名信息','${ctx}/enroll/add','','510')"
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加报名信息</a></span> <span class="r">共有数据：<strong>${enrolllist.size()}</strong>
				条
			</span>
		</div>
		<div class="mt-20">
			<table
				...
				<thead>
					<tr>
						<th scope="col" colspan="8">报名管理</th>
					</tr>
					<tr class="text-c">
				...
				<tbody>
					<c:forEach items="${enrolllist}" var="item">
						<tr class="text-c">
							...
							<td><fmt:formatDate value='${item.enrolltime}'
									pattern='yyyy-MM-dd HH:mm:ss' /></td>
							<td class="f-14"><a title="编辑" href="javascript:;"
								onclick="enroll_edit('报名编辑','${ctx}/enroll/edit?enrollid=${item.enrollid}','${item.enrollid}')"
								style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a title="删除" href="javascript:;"
								onclick="enroll_del(this,${item.enrollid})" class="ml-5"
	...
	$(function() {
	   ...
	   function enroll_add(title,url,w,h){
			
	         layer_show(title,url,w,h);
	   }
		function enroll_edit(title, url, id, w, h) {
			layer_show(title, url, w, h);
		}
		function enroll_del(obj, id) {
			layer.confirm('删除须谨慎，确认要删除吗？', function(index) {
				 $.ajax({
		             type: "GET",
		             url: "${ctx}/enroll/detele?enrollid= "+id,
		             dataType: "json",
