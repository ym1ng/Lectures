...
<body>
	<nav class="breadcrumb">
		...
		讲座管理 <span class="c-gray en">&gt;</span> 收藏管理 <a
	...
	<form action="${ctx}/collection/select">
		<div class="text-c">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <a class="btn btn-primary radius"
				href="javascript:;"
				onclick="collection_add('添加收藏信息','${ctx}/collection/add','800')"><i
					class="Hui-iconfont">&#xe600;</i> 添加收藏信息</a>
			</span> <span class="r">共有数据：<strong>${collectionlist.size()}</strong>
				条
			</span>
		</div>
		<div class="mt-20">
			...
			<table
				...
				<thead>
					<tr>
						<th scope="col" colspan="8">收藏管理</th>
					</tr>
					<tr class="text-c">
					...
					<c:forEach items="${collectionlist}" var="item">
						<tr class="text-c">
							...
							<td class="f-14"><a title="编辑" href="javascript:;"
								onclick="collection_edit('收藏编辑','${ctx}/collection/edit?collectionid=${item.collectionid}','${item.collectionid}')"
								style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a title="删除" href="javascript:;"
								onclick="collection_del(this,${item.collectionid})" class="ml-5"
	...
	$(function() {
	   ...
	   function collection_add(title,url,w,h){
			
	         layer_show(title,url,w,h);
	   }
	
		function collection_edit(title, url, id, w, h) {
			layer_show(title, url, w, h);
		}
		...
		function collection_del(obj, id) {
			layer.confirm('删除须谨慎，确认要删除吗？', function(index) {
				 $.ajax({
		             type: "GET",
		             url: "${ctx}/collection/detele?collectionid= "+id,
		             dataType: "json",
